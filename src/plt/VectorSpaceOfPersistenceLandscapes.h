...
public:
  ...
  vectorSpaceOfPersistenceLandscapes(
    ...
    this->vectOfLand.insert(this->vectOfLand.end(), vectOfLand.begin(),
                            vectOfLand.end());
  }

  void addLandscape(const PersistenceLandscape &aa) {
    this->vectOfLand.push_back(aa);
  }
  void removeLastLandscape() { this->vectOfLand.pop_back(); };

  double computeDiscanceOfLandscapes(
      unsigned p); // computes L^p distance of landscapes. Assume that
                   // this->vectOfLand.size() == 2, othervise works on first two
  double
  computeMaxNormDiscanceOfLandscapes(); // assume that this->vectOfLand.size()
                                        // == 2, othervise works on first two

  std::vector<std::pair<double, double>> averageLandscpceInDiscreteSetOfPoints(
      unsigned dim, unsigned numberOfPoints, double from,
      double to); // compute a vector of discrete values, which are values of
                  // landscapes on discrete set of numberOfPoints points in
                  // interval [ from , to ]
  PersistenceLandscape
  average(); // compute persistence landscape which is average of all the
             // landscapes in this->vectOfLand
...
vectorSpaceOfPersistenceLandscapes::averageLandscpceInDiscreteSetOfPoints(
    unsigned dim, unsigned numberOfPoints, double from, double to) {
  if (numberOfPoints <= 1) {
    std::ostringstream errMessage;
    errMessage << "Error in function "
                  "vectorSpaceOfPersistenceLandscapes::"
                  "averageLandscpceInDiscreteSetOfPoints. You want to compute "
                  "average using : "
               << numberOfPoints
               << " points. This variable cannot be less than 2 \n";
  ...
  if (to <= from) {
    ...
    errMessage << "Error in function "
                  ...
                  "average in intrval ["
               << from << "," << to
               << "]. Please correct the vinterval in a way that it is in a "
                  "form [from,to], where from <to\n";
    std::string errMessageStr = errMessage.str();
    const char *err = errMessageStr.c_str();
    throw(err);
  }
  ...
  for (size_t i = 0; i != numberOfPoints - 1; ++i) {
    result[i] =
        std::make_pair(from + (to - from) * i / (numberOfPoints - 1), 0);
  }
  ...
  // It is O(nlog(n)). In below I am changing it into O(n) algorithm. This
  // version is tested and works well.
  /*
  for ( size_t i = 0 ; i != this->vectOfLand.size() ; ++i )
  {
      ...
      {
          std::cout << "i : " << i << "\n";
          std::cout << "this->vectOfLand[i].dim() : " <<
  this->vectOfLand[i].dim() << "\n"; std::cout << "dim() : " <<
  this->vectOfLand[i].dim() << "\n"; std::cin.ignore();
      }
      ...
      if ( this->vectOfLand[i].dim() >= dim )
      {
          ...
          {
               if (averageLandscpceInDiscreteSetOfPointsBDG)std::cout << "nr : "
  << this->vectOfLand[i].computeValueAtAGivenPoint( dim , result[nr].first ) <<
  "\n"; result[nr].second += this->vectOfLand[i].computeValueAtAGivenPoint( dim
  , result[nr].first );
          }
      }
  }
  */
  ...
  for (size_t i = 0; i != this->vectOfLand.size(); ++i) {
    ...
    if (this->vectOfLand[i].dim() >= dim) {
      size_t nr = 0;
      size_t it = 1;
      while ((nr != numberOfPoints) &&
             (it != this->vectOfLand[i].land[dim].size())) {
        if (averageLandscpceInDiscreteSetOfPointsBDG) {
          std::cout << "nr : " << nr << " , it : " << it
                    << " , numberOfPoints : " << numberOfPoints
                    << " , this->vectOfLand[i].land[dim].size() : "
                    << this->vectOfLand[i].land[dim].size() << std::endl;
          std::cout << "this->vectOfLand[i].land[dim][it-1].first : "
                    << this->vectOfLand[i].land[dim][it - 1].first << std::endl;
          std::cout << "this->vectOfLand[i].land[dim][it].first : "
                    << this->vectOfLand[i].land[dim][it].first << std::endl;
          std::cout << "result[nr].first : " << result[nr].first << std::endl;
          std::cin.ignore();
        }
        if ((this->vectOfLand[i].land[dim][it - 1].first <= result[nr].first) &&
            (this->vectOfLand[i].land[dim][it].first >= result[nr].first)) {
          result[nr].second += functionValue(
              this->vectOfLand[i].land[dim][it - 1],
              this->vectOfLand[i].land[dim][it], result[nr].first);
          ++nr;
          continue;
        }
...
PersistenceLandscape vectorSpaceOfPersistenceLandscapes::average() {
  ...
  if (useGridInComputations) {
    ...
    size_t maxN = 0;
    for (size_t i = 0; i != this->vectOfLand.size(); ++i) {
      if (maxN < this->vectOfLand[i].size()) {
        maxN = this->vectOfLand[i].size();
      }
    }
    if (dbg) {
      cerr << "Maximal nonzero lambda : " << maxN << endl;
    }

    // for every lambda:
    for (size_t lambda = 0; lambda != maxN; ++lambda) {
      if (dbg) {
        cerr << "Considering lambda : " << lambda << endl;
      }
      ...
      while (true) {
        ...
        for (size_t i = 0; i != counter.size(); ++i) {
          if (lambda >= this->vectOfLand[i].land.size())
            continue;
          if (counter[i] >= this->vectOfLand[i].land[lambda].size())
            continue;
          if (this->vectOfLand[i].land[lambda][counter[i]].first < min_x) {
            // we found new point with smaller x coordinate. Whatever we had
            // before that have the be zeored.
            ...
            valueInThisPoint +=
                this->vectOfLand[i].land[lambda][counter[i]].second;
          } else {
            if (this->vectOfLand[i].land[lambda][counter[i]].first == min_x) {
              valueInThisPoint +=
                  this->vectOfLand[i].land[lambda][counter[i]].second;
        ...
        thisLambda.push_back(std::make_pair(
            min_x, valueInThisPoint / (double)this->vectOfLand.size()));
        if (dbg) {
          cerr << "Adding to lambda : " << min_x << " , "
               << valueInThisPoint / (double)this->vectOfLand.size() << endl;
          getchar();
        }
    ...
    // This is an old version of the procedure, in which I am doing a linear
    // mearging. It is not the most efficient one, therefore, I am replacing it
    // with the binary tree like merging.
    /*
    for ( size_t i = 0 ; i != this->vectOfLand.size() ; ++i )
    {
    ...
    */
    ...
    while (nextLevelMerge.size() != 1) {
      if (dbg) {
        cerr << "nextLevelMerge.size() : " << nextLevelMerge.size() << endl;
      }
...
/*
    ...
    if ( useGridInComputations )
    {
        ...
        //first we need to check what is the maximal number of nonzero functions
   in the considered collection of landscapes: size_t maximumNuberOfFunction =
   0; for ( size_t i = 0 ; i != this->vectOfLand.size() ; ++i )
        {
            if ( this->vectOfLand[i].size() > maximumNuberOfFunction
   ){maximumNuberOfFunction = this->vectOfLand[i].size();}
        }
        ...
        for ( size_t lambda = 0 ; lambda != maximumNuberOfFunction ; ++lambda )
        {
            ...
            {
                 for ( size_t numberOfLandscapeToAverage = 0 ;
   numberOfLandscapeToAverage != this->vectOfLand.size() ;
   ++numberOfLandscapeToAverage )
                 {
                     if ( this->vectOfLand[numberOfLandscapeToAverage].size() <
   lambda )continue;
                     ...
                     {
                         cerr << "result.land[lambda][nrOfPoint].first : " <<
   result.land[lambda][nrOfPoint].first << endl; cerr <<
   "this->vectOfLand[numberOfLandscapeToAverage][lambda][nrOfPoint].first : " <<
   this->vectOfLand[numberOfLandscapeToAverage].land[lambda][nrOfPoint].first <<
   endl;
                     }
                     //end of test.
                     result.land[lambda][nrOfPoint].second +=
   this->vectOfLand[numberOfLandscapeToAverage].land[lambda][nrOfPoint].second;
                 }
                 result.land[lambda][nrOfPoint].second /=
   (double)this->vectOfLand.size();
            }
        }
    }
*/
...
double
vectorSpaceOfPersistenceLandscapes::standardDeviation(int whichDistance) {
  ...
  for (size_t i = 0; i != this->vectOfLand.size(); ++i) {
    ...
    if (whichDistance > 0) {
      ...
      distance = ::computeDiscanceOfLandscapes(av, this->vectOfLand[i],
...
(unsigned)whichDistance );
        ...
        {
            //L^infty distance
            distance = ::computeMaxNormDiscanceOfLandscapes( av ,
this->vectOfLand[i] );
        }
        distanceToAverage += distance*distance;
    }
