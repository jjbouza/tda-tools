...
int main(int argc, char *argv[]) {
  ...
  if (argc != numberOfInputFiles + 4) {
    ...
    std::cout
        << "Wrong usage of a program. Please call <program name> <positive "
           "integer N equal to the number of files with input files names>";
    std::cout << "N times <name of the file with names of files with "
                 "persistence landscapes/barcodes> ";
    std::cout << "<positive integer equal to the number of permutations in the "
                 "permutation test> ";
  ...
  std::cout
      << "Number of files with list of files with barcodes and landscapes : "
      << numberOfInputFiles << std::endl;
  for (size_t i = 0; i != inputFiles.size(); ++i) {
    std::cout << inputFiles[i] << std::endl;
  }
  std::cout << "Number of permutations with the permutation test : "
            << numberOfPermutations << std::endl;
  ...
  std::cout << "Reading the names of files with barcodes \\ landscapes."
            << std::endl;
  ...
  for (size_t i = 0; i != namesOfFiles.size(); ++i) {
    std::vector<PersistenceLandscape> lands =
        createLandscapesFromTheFiles(namesOfFiles[i]);
  ...
  std::vector<std::pair<std::pair<size_t, size_t>, size_t>>
      resultOfPermutationTest;
  if (whichDistance != -1) {
    // L^p distance
    resultOfPermutationTest =
        permutationTest(landscapes, numberOfPermutations, whichDistance);
  } else {
    // bottleneck distance
    resultOfPermutationTest =
        permutationTest(landscapes, numberOfPermutations, whichDistance, true);
  }
  std::cout << "Done with the permutation test, writing p-values to the file "
               "'p-values.txt."
            << std::endl;
  ...
  for (size_t i = 0; i != resultOfPermutationTest.size(); ++i) {
    pValues.push_back(std::make_pair(resultOfPermutationTest[i].first,
                                     resultOfPermutationTest[i].second /
