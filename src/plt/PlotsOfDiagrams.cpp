...
int main(int argc, char *argv[]) {
  ...
  if (argc != 2) {
    ...
    std::cout << "Wrong usage of a program. Please call <program name> "
              << std::endl;
    std::cout
        << "<name of the file with names of files with persistence barcodes> "
        << std::endl;
  ...
  std::cout << "Name of the file with barcodes : " << fileWithInputFiles
            << std::endl;
  ...
  std::vector<PersistenceBarcodes> barcodes =
      createBarcodesFromTheFiles(namesOfFiles);
