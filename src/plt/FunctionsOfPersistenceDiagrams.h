...
double maxLengthsOfInterval(const PersistenceBarcodes &b) {
  ...
  for (size_t i = 0; i != b.barcodes.size(); ++i) {
    if (maxLength < fabs(b.barcodes[i].second - b.barcodes[i].first))
      maxLength = fabs(b.barcodes[i].second - b.barcodes[i].first);
  }
  return maxLength;
}
...
double lengthIfIthBar(const PersistenceBarcodes &b, size_t position) {
  if (b.size() > position)
    return (b.barcodes[position].second - b.barcodes[position].first);
  return 0;
}

double medianLengthsOfDominantIntervals(const PersistenceBarcodes &b,
                                        size_t numberOfDominantIntervals) {
  if (b.size() == 0)
    return 0;
// cerr << "medianLengthsOfDominantIntervals \n";
std:
  vector<double> result(std::min(b.size(), numberOfDominantIntervals));
...
double averageLengthsOfDominantIntervals(const PersistenceBarcodes &b,
                                         size_t numberOfDominantIntervals) {
  if (b.size() == 0)
    return 0;
