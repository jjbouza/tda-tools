...
int main(int argc, char **argv) {
  ...
  if (numberOfClasses < 1) {
    std::cout
        ...
        << " which is not a correct value. The program will terminate now."
        << std::endl;
    return 1;
  }
  ...
  for (size_t classNo = 0; classNo != numberOfClasses; ++classNo) {
    ...
    std::vector<std::string> namesOfFilesFirstFamilly =
        readFileNames(argv[2 + classNo]);
    if (namesOfFilesFirstFamilly.size() == 0) {
      std::cerr << "Class number : " << classNo
                << " is empty. This is not a correct input to the program. "
                   "Please remove the class or correct the file name. The "
  ...
  std::cerr << "Names of files with different landscapes or barcodes classes:"
            << std::endl;
  ...
  for (size_t classNo = 0; classNo != classes.size(); ++classNo) {
    std::vector<PersistenceLandscape> landscapesThisFamilly =
        createLandscapesFromTheFiles(classes[classNo]);
