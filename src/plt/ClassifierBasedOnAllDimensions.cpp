...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Number of dimensions of persistence to consider : "
            << numberOfDimensionsToConsider << std::endl;
  ...
  if (action == 1) {
    ...
    persistenceClassifierAllDimension c(landscapes,
                                        numberOfDimensionsToConsider);
    c.writeToFile((char *)"class");
    std::cout << "Averages computed and written to the folder. The program "
                 "will now terminate."
              << std::endl;
  } else {
    if (action == 2) {
      ...
      if (argc != 7) {
        ...
        std::cout << "A positive integer N indicating how many classes there "
                  ...
                  << std::endl;
        std::cout << "A file with names of files which are to be classified,"
                  << std::endl;
        std::cout
            << "An positive real number p indicating which distance is to be "
               "used in classification (set -1 for the bottleneck distance),"
            << std::endl;
        // std::cout << "A 0-1 variable q. If q = 0 for each input barcode or
        // landscape the best matching cluster will be computed. If q = 1, the
        // distances to all averages will be given," << std::endl;
      ...
      c.readFromFile((char *)"class", numberOfClassesToClassify,
                     numberOfDimensionsToConsider, usedDistance);
      cerr << "End reading \n";
