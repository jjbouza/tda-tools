...
permutationTest(const std::vector<std::vector<PersistenceLandscape>> data,
                size_t numberOfTries, unsigned whichDistance,
                bool setThisToTrueIfYouWantBottleneck = false) {
  bool dbg = false;

  std::vector<std::pair<std::pair<size_t, size_t>, size_t>> result;

  if (dbg) {
    cerr << "Entering permutationTest \n";
  }
  ...
  if (dbg) {
    cerr << "Averages computed \n";
  }
  // now the main bootstraping:
  for (size_t i = 0; i != data.size(); ++i) {
    for (size_t j = i + 1; j != data.size(); ++j) {
      cerr << "Comparing the landscapes from the file number : " << i + 1
           << " , and the file number: " << j + 1 << ". " << numberOfTries
           << " permutations will be made." << endl;
      ...
      if (setThisToTrueIfYouWantBottleneck = false) {
        ...
        distanceBetweenOriginal = computeDiscanceOfLandscapes(
            averages[i], averages[j], whichDistance);
      } else {
        // in this case we compute bottleneck distance.
        distanceBetweenOriginal =
            computeMaxNormDiscanceOfLandscapes(averages[i], averages[j]);
      }
      ...
      for (size_t tryy = 0; tryy != numberOfTries; ++tryy) {
        ...
        for (size_t a = data[i].size(); a != data[i].size() + data[j].size();
             ++a) {
        ...
        if (setThisToTrueIfYouWantBottleneck = false) {
          // in this case we compute L^p distance
          distanceBetweenPermutted =
              computeDiscanceOfLandscapes(avv0, avv1, whichDistance);
        } else {
          // in this case we compute bottleneck distance.
          distanceBetweenPermutted =
              computeMaxNormDiscanceOfLandscapes(avv0, avv1);
        }
      ...
      std::cerr << "How many averages of permutted sets are farther away than "
                   "the averages of the original sets: "
                << howManyPermuttedAveragesIsGreaterThanOriginal << std::endl;
      result.push_back(std::make_pair(
          std::make_pair(i, j), howManyPermuttedAveragesIsGreaterThanOriginal));
    }
  }
...
template <typename T>
void writeToFileAsMatrix(std::vector<std::pair<std::pair<size_t, size_t>, T>> b,
                         char *filename) {
  ofstream out;
  out.open(filename);
...
template <typename T>
void writeToFileAsAList(std::vector<std::pair<std::pair<size_t, size_t>, T>> b,
                        char *filename) {
  ofstream out;
  out.open(filename);
  for (size_t i = 0; i != b.size(); ++i) {
    out << "(" << b[i].first.first + 1 << "," << b[i].first.second + 1
        << ") = " << b[i].second << std::endl;
  }
  out.close();
}
