...
const char *programInfo =
    "This is PlotsOfLandscape program which is a part of Peristence Landscape "
    "Toolbox by Pawel Dlotko. It takes as input the name of a file with "
    "landscape or barcode and two integer parameters indicating what range of "
    "barcodes is to be plotted.";
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Name of the file with barcode \\ landscape : "
            << fileWithLandscape << std::endl;
  ...
  std::vector<PersistenceLandscape> landscapes =
      createLandscapesFromTheFiles(names);
  ...
  if (t != -1) {
    ...
    landscapes[0].generateGnuplotCommandToPlot(fileWithLandscape, (unsigned)f,
                                               (unsigned)t);
  } else {
    landscapes[0].printToFiles(fileWithLandscape, (unsigned)f,
                               (unsigned)landscapes[0].size());
    if (f == 0) {
      landscapes[0].generateGnuplotCommandToPlot(fileWithLandscape);
    } else {
      landscapes[0].generateGnuplotCommandToPlot(
          fileWithLandscape, (unsigned)f, (unsigned)landscapes[0].size());
    }
  }
