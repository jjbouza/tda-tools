...
// This procedure is designed to read file names from a file provided by user
// and return vector of those names.
std::vector<std::string> readFileNames(char *filenameWithFilenames) {
  ...
  while (!in.eof()) {
    ...
    if ((line.length() == 0) || (line[0] == '#')) {
      // in this case we have a file name. First we should remove all the white
      // spaces.
      if (dbg) {
        std::cerr << "This is a line with comment, it will be ignored n";
      }
    } else {
      result.push_back(line);
      if (dbg) {
        std::cerr << "Line after removing white spaces : " << line << std::endl;
      }
    }
  }
  in.close();
  ...
  /*
  std::cout << "Read the following files from the file : " <<
  filenameWithFilenames << std::endl; for ( size_t i = 0 ; i != result.size() ;
  ++i )
  {
...
// This procedure reads all the files named in the vector<string>. It can read
// both barcode files and landscape files. But in the case of landsape files, we
// assume that the extension is
//'.lan'.
std::vector<PersistenceLandscape>
createLandscapesFromTheFiles(std::vector<std::string> filenames) {
  bool dbg = true;
  ...
  for (size_t fileNo = 0; fileNo != filenames.size(); ++fileNo) {
    ...
    if (dbg) {
      std::cerr << "Considering a file : " << filename << std::endl;
    }
    if (dbg) {
      std::cerr << "isThisBarcodeFile : " << isThisBarcodeFile << std::endl;
    }

    if (isThisBarcodeFile) {
      if (dbg) {
        std::cerr << "This is a barcode file" << std::endl;
      }
      ...
      if (dbg) {
        std::cerr << "This is a landscape file" << std::endl;
      }
...
// This procedure reads all the files named in the vector<string>. They all
// asume to store barcode
std::vector<PersistenceBarcodes>
createBarcodesFromTheFiles(std::vector<std::string> filenames) {
  bool dbg = true;
