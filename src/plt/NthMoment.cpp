...
int main(int argc, char *argv[]) {
  ...
  if (argc != 4) {
    ...
    std::cout << "<name of the file with persistence landscapes/barcodes> "
              << std::endl;
    std::cout
        << "<A positive integer indicating which moment we are to compute>."
        << std::endl;
    std::cout << "<A positive integer l indicating which landscape's moment we "
                 "are to compute>."
              << std::endl;
  ...
  double moment =
      normalizingFactor * landscapes[0].computeNthMoment(n, center, lambda);
