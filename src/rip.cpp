// TODO:
// 1. Extend to accept point clouds, images. (Consider extending to accept pre-computed boundary matrices (i.e. a simplicial complex).)
...
//Note: We are currently returning a std::vector. I think Rcpp performs a deep copy of this output vector to an R
//NumericVector type. This could be expensive for very large complexes with tens of thousands of persistence pairs.
//The issue with using a NumericVector internally is that NumericVector does not support dynamic resizing, which is
//central to the Ripser algorithm.

//N = number of points (i.e. D_R is of size N^2)
//D = LOWER-TRIANGULAR distance matrix (i.e. all of the entries above the diagonal can be set to zero).

// [[Rcpp::export]]
std::vector<double> rip(NumericMatrix D_R, int N, int modulus, int dim_max, float threshold, int do_cocycles) {
	//Convert input distance matrix to Ripser format.
	std::vector<value_t> distances(N*N);
	//i iterates over rows, j over cols.
	for(int j = 1; j < N; j++)
		for(int i = 0; i < j; i++)
			distances[i*(j+1)] = D_R(i,j);
