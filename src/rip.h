...
// Set correct compilation flags for use in R.
#define USE_COEFFICIENTS 1
#define ASSEMBLE_REDUCTION_MATRIX 1
...
class hash_map : public std::unordered_map<Key, T> {};
...
bool is_prime(const coefficient_t n) {
  if (!(n & 1) || n < 2)
    return n == 2;
  for (coefficient_t p = 3, q = n / p, r = n % p; p <= q;
       p += 2, q = n / p, r = n % p)
    if (!r)
      return false;
  return true;
...
// static_assert(sizeof(entry_t) == sizeof(index_t), "size of entry_t is not the
// same as index_t");
...
bool operator==(const entry_t &e1, const entry_t &e2) {
  return get_index(e1) == get_index(e2) &&
         get_coefficient(e1) == get_coefficient(e2);
}
...
public:
  ...
  diameter_entry_t(value_t _diameter, index_t _index,
      ...
      : std::pair<value_t, entry_t>(_diameter,
                                    make_entry(_index, _coefficient)) {}
  diameter_entry_t(const diameter_index_t &_diameter_index,
                   coefficient_t _coefficient)
      : std::pair<value_t, entry_t>(
...
public:
  ...
  sparse_distance_matrix(int *I, int *J, float *V, int NEdges, int N,
                         float threshold)
      : neighbors(N) {
    int i, j;
    value_t val;
...
template <>
value_t compressed_distance_matrix<UPPER_TRIANGULAR>::
operator()(const index_t i, const index_t j) const {
  return i == j ? 0 : i > j ? rows[j][i] : rows[i][j];
}

template <>
value_t compressed_distance_matrix<LOWER_TRIANGULAR>::
operator()(const index_t i, const index_t j) const {
  return i == j ? 0 : i < j ? rows[j][i] : rows[i][j];
}
...
diameter_entry_t pop_pivot(Heap &column, coefficient_t modulus) {
  if (column.empty())
...
template <typename Heap>
diameter_entry_t get_pivot(Heap &column, coefficient_t modulus) {
...
public:
  ...
  void append_column(Iterator begin, Iterator end) {
  ...
  void append_column(const Collection collection) {
...
typedef struct {
  ...
  */
  ...
  /*
    ...
    dimension. For now, only cocycles above dimension 0 are added, so dimension
    0 is an empty list For the others, cocycles_by_dim[d] holds an array of
    representative cocycles for dimension d which is parallel with the array of
    births/deaths for dimension d. Each element of the array is itself an array
    of unrolled information about the cocycle For dimension 1, for example, the
    zeroeth element of the array contains [ccl0_simplex0_idx0 ccl0_simplex0_idx1
    ccl0_simplex0_val, ccl0_simplex1_idx0 ccl0_simplex1_idx1 ccl0_simplex1_val,
    ... ccl0_simplexk_idx0 ccl0_simplexk_idx1 ccl0_simplexk_val] for a cocycle
    representing the first persistence point, which has k simplices with nonzero
    values in the representative cocycle
  */
...
template <typename DistanceMatrix> class ripser {
  ...
  int do_cocycles; // If this flag is off, don't extract the representative
                   // cocycles to save time
...
public:
  ...
  diameter_entry_t
  add_coboundary_and_get_pivot(Iterator column_begin, Iterator column_end,
    ...
    for (auto it = column_begin; it != column_end; ++it) {
      ...
      set_coefficient(simplex, get_coefficient(simplex) * factor_column_to_add %
                                   modulus);
      ...
      for (auto coface : coface_entries)
    ...
    for (size_t index_column_to_reduce = 0;
      ...
      while (true) {
        ...
        for (auto it = reduction_matrix.cbegin(index_column_to_add);
          ...
          coeffs.push_back(*it);
        auto reduction_column_begin = coeffs.begin(),
             reduction_column_end = coeffs.end();
#endif
#else
#ifdef USE_COEFFICIENTS
        ...
        if (get_index(pivot) != -1) {
          ...
          if (pair != pivot_column_index.end()) {
...
#ifdef ASSEMBLE_REDUCTION_MATRIX
            // replace current column of reduction_matrix (with a single
            // diagonal 1 entry) by reduction_column (possibly with a different
            ...
            reduction_matrix.push_back(
                diameter_entry_t(column_to_reduce, inverse));
#endif
#endif
            break;
          }
...
private:
  ...
  std::vector<index_t> &vertices;
  std::vector<std::vector<diameter_index_t>::const_reverse_iterator>
      &neighbor_it;
  std::vector<std::vector<diameter_index_t>::const_reverse_iterator>
      &neighbor_end;
  diameter_index_t x;
...
public:
  simplex_coboundary_enumerator(const diameter_entry_t _simplex, index_t _dim,
      ...
      : parent(_parent), idx_below(get_index(_simplex)), idx_above(0),
        v(parent.n - 1), k(_dim + 1), max_vertex_below(parent.n - 1),
        simplex(_simplex), modulus(parent.modulus), dist(parent.dist),
        binomial_coeff(parent.binomial_coeff), vertices(parent.vertices),
  ...
  bool has_next(bool all_cofaces = true) {
    for (auto &it0 = neighbor_it[0], &end0 = neighbor_end[0]; it0 != end0;
      ...
      return all_cofaces ||
             !(k > 0 && parent.get_next_vertex(max_vertex_below, idx_below, k) >
                            get_index(x));
    continue_outer:;
    }
    return false;
  ...
  diameter_entry_t next() {
    ...
    while (k > 0 && parent.get_next_vertex(max_vertex_below, idx_below, k) >
                        get_index(x)) {
    ...
    return diameter_entry_t(coface_diameter,
                            idx_above + binomial_coeff(get_index(x), k + 1) +
                                idx_below,
                            coface_coefficient);
  }
};
...
std::vector<diameter_index_t> ripser<sparse_distance_matrix>::get_edges() {
  ...
  for (index_t i = 0; i < n; ++i)
    for (auto n : dist.neighbors[i]) {
      ...
      if (i > j)
        edges.push_back(std::make_pair(get_diameter(n), get_edge_index(i, j)));
    }
  return edges;
}
...
Rcpp::List ripserResultToR(ripserResults inp) {
  ...
  // We want to return cocycles as a list (dim) of lists (PD point) of matricies
  // (repinpesentitive).
  int dim_cnt = 2;
  for (auto dim : inp.cocycles_by_dim) {
    std::vector<Rcpp::NumericVector> dim_vec =
        std::vector<Rcpp::NumericVector>();
    for (auto point : dim) {
      Rcpp::NumericVector rep = Rcpp::wrap(point);
      rep.attr("dim") =
          Rcpp::Dimension(static_cast<int>(point.size() / dim_cnt), dim_cnt);
...
Rcpp::List rip_raw(std::vector<float> &distances, int modulus, int dim_max,
  ...
  if (threshold == std::numeric_limits<value_t>::max() || threshold == -1)
  ...
  if (threshold >= max) {
    ...
    ripser<sparse_distance_matrix> r(
        sparse_distance_matrix(std::move(dist), threshold), dim_max, threshold,
...
ripserResults rips_dm_sparse(int *I, int *J, float *V, int NEdges, int N,
  ...
  float ratio = 1.0; // TODO: This seems like a dummy parameter at the moment
  ripser<sparse_distance_matrix> r(
      sparse_distance_matrix(I, J, V, NEdges, N, threshold), dim_max, threshold,
      ratio, modulus, do_cocycles);
  r.compute_barcodes();
